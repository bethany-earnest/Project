...
us <- readOGR(dsn="us.geo.json", layer="OGRGeoJSON")
...
# us_map <- fortify(us_aea, region="GEOID")
us_map <- fortify(us_aea)
# plot it
gg <- ggplot()
gg <- gg + geom_map(data=us_map, map=us_map,
                    aes(x=long, y=lat, map_id=id, group=group),
                    fill="white", color="white", size=0.15)
gg <- gg + geom_map(data=us_aea@data, map=us_map, aes(map_id=GEOID, fill=log(ALAND)),
...
map("state", interior = FALSE)
map("state", boundary = FALSE, col="gray", add = TRUE)
#map("world", c("USA","hawaii","alaska"), xlim=c(-180,-65), ylim=c(19,72),interior = FALSE)
...
A summary table listing number of shark attacks ocurred in each state.
...
result <- as.data.frame(table(as.character(over(points, usa.states)$STATE_NAME)))
...
```{r}
my_data <- cbind(result$count, geocode(as.character(result$state)))
colnames(my_data) <- c("count","lon","lat")
...
USAMap + 
   geom_point(aes(x=lon, y=lat), data=my_data, col="red", alpha=0.4,
              size=my_data$count*circle_scale_amt) +
              scale_size_continuous(range=range(my_data$count))
dev.off()
# US map
...
Data can be downloaded at http://www.ncdc.noaa.gov/oisst.
...
```{r, echo=FALSE}
# lonW=80,lonE=110,latS=17,latN=35 has values -- A test case
