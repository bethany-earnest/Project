...
USA_tab <- SA_tab %>% filter(Country=="USA") %>%
  select(CaseNumber,Location,Area,Country,Date) 

if (any( is.na(USA_tab))) {
  USA_tab[is.na(USA_tab)] <- ""
}
address <- do.call(paste, c(select(USA_tab,-c(CaseNumber,Date)), 
                                   sep=" "))
address <- iconv(address,from="CP1252",to="UTF-8")

locations <- ldply(address,
                   function(x) geoCode(x))
loc <- data.frame(USA_tab$CaseNumber,USA_tab$Date,locations) %>%
  na.omit()
