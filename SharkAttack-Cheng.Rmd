...
USA_tab <- SA_tab %>% filter(Country=="USA") %>%
  select(Location,Area,Country,Date) 
if (any( is.na(USA_tab))) {
  USA_tab[is.na(USA_tab)] <- ""
}
address <- do.call(paste, c(USA_tab, sep=" "))

locations <- ldply(address[1:50],
                   function(x) geoCode(x))
loc <- data.frame(USA_tab$Date[1:50],locations) %>% na.omit()
colnames(loc) <- c("day","lat","lon","location_type","forAddress")
...
```{r}
mean_temp <- function(lon,lat,day) {
  lonarg <- c(floor(lon),ceiling(lon))
  latarg <- c(floor(lat),ceiling(lat))
  ssts = extractOISSTdaily("daily/sst.day.mean.2015.v2.nc",
...
loc$day <- as.Date(as.character(loc$day),format="%d-%b-%y")
loc <- na.omit(loc)

lon <- as.numeric(loc$lon)+180
lat <- as.numeric(loc$lat)
day <- loc$day
loc$temp <- lapply(lon, mean_temp,lat=lat,day=day)
```
